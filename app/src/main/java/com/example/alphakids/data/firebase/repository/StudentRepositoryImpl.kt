...
class StudentRepositoryImpl @Inject constructor(
    ...
    // Referencia compartida a la colección de estudiantes para evitar escribir el nombre una y otra vez.
    private val estudiantesCol = db.collection("estudiantes")

    // Datos determinísticos que permiten probar la interfaz incluso cuando el tutor no tiene estudiantes reales todavía.
    ...
    override suspend fun createStudent(estudiante: Estudiante): CreateStudentResult {
        return try {
            // Guardamos el documento en Firestore y devolvemos el identificador asignado por la base de datos.
    ...
    override fun getStudentsForTutor(tutorId: String): Flow<List<Estudiante>> {
        ...
        return query.snapshots().map { querySnapshot ->
            ...
            // Si hay un error en la consulta devolvemos la lista mock para que la UI continúe operativa.
    ...
    override suspend fun getStudentById(studentId: String): Estudiante? {
        return try {
            // Consultamos el documento para obtener siempre la versión más reciente del estudiante.
    ...
    override suspend fun updateStudent(estudiante: Estudiante): Result<Unit> {
        if (estudiante.id.isBlank()) {
            // Evitamos ejecutar escrituras con identificadores inválidos porque Firestore generaría un nuevo documento.
